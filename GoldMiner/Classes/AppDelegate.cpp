...
bool AppDelegate::applicationDidFinishLaunching() {
	...
	auto createAnimation = [](int totalFrames, String frame, String name) -> void {
		char frameName[20];
		Animation* animation = Animation::create();
		for (int i = 0; i < totalFrames; i++) {
			sprintf(frameName, frame.getCString(), i);
			animation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(frameName));
		}
		animation->setDelayPerUnit(0.1);
		AnimationCache::getInstance()->addAnimation(animation, name.getCString());
	};

	createAnimation(3, "miner-leg-%d.png", "legAnimation");
	createAnimation(3, "cave-%d.png", "caveAnimation");

	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("level-sheet.plist");
	createAnimation(8, "gem-mouse-%d.png", "mouseAnimation");
	createAnimation(7, "gold-0-%d.png", "goldAnimation");
	createAnimation(7, "diamond-%d.png", "diamond");
